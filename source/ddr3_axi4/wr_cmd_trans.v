// AXI4 write command translator — converts pixel write requests into AXI4 burst write transactions
...
module wr_cmd_trans #(
    ...
    begin
        if(~rstn)
            wr_data_req_reg <= 1'b0;
        else if(wr_ready) 
        begin  
            if(burst_cnt == wr_len && wr_data_req_reg)
                wr_data_req_reg <= 1'b0;
            else
                wr_data_req_reg <= 1'b1;
        end 
        else
            wr_data_req_reg <= 1'b0;
    end
    assign wr_data_req = wr_ready;// & wr_data_req_reg;
